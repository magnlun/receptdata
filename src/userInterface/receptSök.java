...
public class receptSök extends JFrame implements ActionListener, Ruta{
	...
	class temp extends JPanel implements MouseListener{
		...
		class temp2 extends JPanel{
			...
			temp2(){
				...
				try {
					String[][] kategorier = Hämta.hämtning("SELECT * FROM \"Kategorier\";");
		...
		public void mousePressed(MouseEvent e) {
			...
			if(recept.getValueAt(row, col) != null){
				...
				try {
					...
					button.changeList(new receptSök());
					new receptFönstret(Ingredienser, recept);
	...
	public String[][] query(String[] ingrediens, String[] Inteingrediens, String Kategori) throws SQLException{
		...
		for(int i = 0; i < ingrediens.length; i++){
			...
			String[][] svar = Hämta.hämtning(query);
			if(svar.length > 1){
